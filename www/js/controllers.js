angular.module('starter.controllers', [])
  ...
  //交易公告
  .controller('DealNoticeCtrl', function ($scope, $rootScope, $stateParams, $state, BooLv, CommonService, MainService, WeiXinService) {
    CommonService.ionicLoadingShow();
  ...
  //公司新闻
  .controller('CompanyTrendsCtrl', function ($scope, $rootScope, $stateParams, $state, BooLv, CommonService, MainService, WeiXinService) {
    CommonService.ionicLoadingShow();
    $scope.getNewsDetails = function () {
      var Id = $stateParams.Id;
      MainService.getNews(Id).success(function (data) {
  ...
  //我的账号
  .controller('AccountCtrl', function ($scope, $rootScope, $state, BooLv, CommonService, AccountService, MainService, WeiXinService) {
    //是否登录
    if (!CommonService.isLogin(true)) {
      return;
    }
  ...
  //H5芝麻信用授权回调解析参数
  .controller('ZmCallBackCtrl', function ($scope, $rootScope, $state,$stateParams, $location,CommonService, AccountService) {
    if (!ionic.Platform.isWebView()) { //如果是H5浏览器页面或者微信  Check if we are running within a WebView (such as Cordova)
        //获取芝麻信用的返回数据传到服务器端解密再获取openId
        $scope.params = {
          userid: localStorage.getItem("usertoken"), //用户id
          params: $location.search()  //芝麻信用回调地址返回参数url?params=xxx&sign=xxx   json对象
        }
